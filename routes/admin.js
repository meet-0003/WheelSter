...
router.put("/approve-vehicle/:vehicleId", authenticateToken, authorizeRole(["admin"]), async (req, res) => {
    try {
        ...
        const vehicle = await Vehicle.findById(req.params.vehicleId).populate("addedBy", "email username");
...
router.get("/company-data", authenticateToken, authorizeRole(["admin"]), async (req, res) => {
    try {
        ...
        const totalRevenue = await Payment.aggregate([
            ...
            { $match: { "bookingDetails.status": { $ne: "Cancelled" } } },
        ...
        // Fetch revenue distribution by vehicle type with admin profit calculation (excluding cancelled bookings)
...
router.get("/bookings", authenticateToken, async (req, res) => {
    try {
        ...
        if (req.user.role === "user") {
            query.user = req.user.id; // Users see their own bookings
        } else if (req.user.role === "driver") {
            // Get vehicles the driver owns
            const driverVehicles = await Vehicle.find({ owner: req.user.id }).select("_id");

            query = {
                $or: [
                    { user: req.user.id }, // Bookings made by driver
                    { vehicle: { $in: driverVehicles.map(v => v._id) } }, // Bookings for their vehicles
                ]
            };
        }
        // Admins see everything (no change needed)

        const bookings = await Booking.find(query)
            .populate("user", "username email")
            .populate("vehicle", "name")
            .populate("driver", "username email phnumber avatar driverInfo")
            .select("status pickupTime duration driverStatus totalAmount createdAt")
            .sort({ createdAt: -1 })
            .lean();

        // Add refunded amount
        for (let booking of bookings) {
            const payment = await Payment.findOne({ booking: booking._id }).select("refundedAmount").lean();
            booking.refundedAmount = payment?.refundedAmount || 0;
        }
...
// Update booking status (admin only) done
router.put("/bookings-status", authenticateToken, authorizeRole(["admin"]), async (req, res) => {
    try {
